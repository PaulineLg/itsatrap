...
public:
    ...
    void specificAttrib(){
        if(m_vertexAttribPos != -1){
            glVertexAttribPointer(m_vertexAttribPos, 3,GL_FLOAT, GL_FALSE, sizeof(glimac::ShapeVertex), (const GLvoid*) offsetof(glimac::ShapeVertex, position));
            //glVertexAttribPointer(m_vertexAttribPos, 3,GL_FLOAT, GL_FALSE, sizeof(type), (const GLvoid*)0);
        }
        if(m_vertexAttribNormal != -1){
            glVertexAttribPointer(m_vertexAttribNormal, 3, GL_FLOAT, GL_FALSE, sizeof(glimac::ShapeVertex), (const GLvoid*) offsetof(glimac::ShapeVertex, normal));
        ...
        if(m_vertexAttribTex != -1){
            glVertexAttribPointer(m_vertexAttribTex, 2, GL_FLOAT, GL_FALSE, sizeof(glimac::ShapeVertex), (const GLvoid*) offsetof(glimac::ShapeVertex, texCoords));
